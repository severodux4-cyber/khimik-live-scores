...
TEST_NOTIFY = os.getenv("TEST_NOTIFY", "").lower() in (
    "1",
    "true",
    "yes",
    "on",
...
# После начала матча считаем его завершённым,
# если страница не даёт признаков текущей игры
...
session.headers.update({
    "User-Agent": (
        ...
        "AppleWebKit/537.36 KhimikScoresBot/7.0"
    )
})
...
def load_users():
    ...
    if USERS_FILE.exists():
        try:
            ...
            logging.warning(
                "USERS: не удалось прочитать users.json: %s",
                e,
            )
...
def process_telegram_commands(users):
    ...
    при очередном запуске GitHub Actions.
    """
    ...
    try:
        ...
        logging.warning(
            "TELEGRAM UPDATES: ошибка получения команд: %s",
            e,
        )
        return
    ...
    if not payload.get("ok"):
        logging.warning(
            "TELEGRAM UPDATES: API вернул ошибку: %s",
            payload,
        )
        return
    ...
    for update in payload.get("result", []):
        ...
        if command == "/start":
            ...
            try:
                ...
                logging.info(
                    "USERS: подключён chat_id=%s",
                    chat_id,
                )

            except Exception as e:
                logging.warning(
                    "USERS: не удалось отправить приветствие %s: %s",
                    chat_id,
                    e,
                )
            ...
            try:
                ...
                logging.info(
                    "USERS: отключён chat_id=%s",
                    chat_id,
                )

            except Exception as e:
                logging.warning(
                    "USERS: не удалось отправить /stop %s: %s",
                    chat_id,
                    e,
                )
            ...
            try:
                ...
                logging.warning(
                    "USERS: не удалось отправить /help %s: %s",
                    chat_id,
                    e,
                )
...
def telegram(text):
    ...
    для тестовой отправки администратору.
    """
...
def broadcast(text, users):
    ...
    for chat_id, info in list(
        ...
        try:
            ...
            logging.warning(
                "BROADCAST: ошибка chat_id=%s: %s",
                chat_id,
                e,
            )

    logging.info(
        "BROADCAST: отправлено пользователям: %d",
        sent,
    )
...
def load_state():
    ...
    try:
        ...
        if meta.get("version") not in (
            ...
            logging.info(
                "STATE: старый формат, создаём новый baseline"
            )
            return {}

        INITIAL_NOTIFY_DONE = bool(
            meta.get(
                "initial_notify_done",
                False,
            )
...
def discover_group(group_url):
    ...
    Возвращаем:
      ...
      - ссылку на страницу команды Химика.
    """
...
def parse_score(soup):
    ...
    # 2. Live:
    # scoreboard может быть 0:0,
    # а голы уже есть в событиях.
    ...
    home = goal_count(
        ".cub-event.team1-event .popup-title"
    )

    away = goal_count(
        ".cub-event.team2-event .popup-title"
    )
...
def parse_status(
    ...
    """
    Определяет статус без ложного
    '3 период' из названий вкладок.

    На странице ФХМО текст '3 период'
    может присутствовать просто как
    название вкладки ленты.

    Поэтому сначала смотрим
    на фактические события игры,
    затем на время от начала.
    """
    ...
    # Берём период из фактических событий,
    # а НЕ из текста кнопок-вкладок.
...
def parse_match(
    ...
    if score is None:
        raise ValueError(
            "Не удалось определить счёт"
        )

    date_time, scheduled_dt = parse_datetime(
        soup,
        page_text,
    )
...
def main():
    ...
    if TEST_NOTIFY:
        ...
        logging.info(
            "TEST_NOTIFY: тестовое сообщение отправлено в Telegram"
        )
    ...
    for age, _label, age_url in ages:
        ...
        for group_label, group_url in groups:
            ...
            try:
                ...
                # Для 2010–2016 используем
                # страницу команды.
                if age != "2017" and team_url:

                    links = discover_team_matches(
                        team_url
                    )
    ...
    if not INITIAL_NOTIFY_DONE:
        ...
        logging.info(
            "INITIAL_NOTIFY: отправляем все уже сыгранные и текущие результаты"
        )
    ...
    for (
        ...
        if initial_mode:
            ...
            # получает только владелец бота.

            telegram(message)

        else:

            # После первичной рассылки
    ...
    # State обновляем после успешной
    ...
    if not INITIAL_NOTIFY_DONE:
        ...
        logging.info(
            "INITIAL_NOTIFY: первая рассылка завершена, дальше только изменения счёта"
        )
